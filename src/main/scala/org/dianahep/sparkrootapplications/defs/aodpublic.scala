...
object aodpublic {
  /*
   * Explicit definitions for most common classes used.
   * Public AOD Muonia Dataset
   */
  case class Empty();
  case class Coordinates(x: Float, y: Float, z: Float);
  case class Vertex(cs: Coordinates);
  case class RecoLeafCandidate(base: Empty, qx3: Integer, pt: Float,
                              eta: Float, phi: Float, mass: Float,
                              vertex: Vertex, pdfId: Integer, status: Integer,
